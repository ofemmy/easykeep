...
const New2 = ({ user }) => {
  ...
  let formType: TrxFrequency;
  if (type == "once") {
    formType = TrxFrequency.Once;
  } else if (type == "recurring") {
    formType = TrxFrequency.Recurring;
  }
  const { formComponent, initialValues, schema } = useFormConfig(formType);
  useEffect(() => {
    setUser(user);
  }, [user]);
  useEffect(() => {
            ...
            <div className="flex items-start justify-between space-x-3">
              <div className="space-y-2">
                <h2 id="slide-over-heading" className="text-lg font-medium ">
                  {`New ${
                    formType == TrxFrequency.Recurring ? "Recurring" : ""
                  } Transaction`}
                </h2>
                <div className="flex items-center">
                  <button
                    ...
                    onClick={() => {
                      router.push(
                        `/new2?type=${
                          formType == TrxFrequency.Once ? "recurring" : "once"
                        }`
                      );
                    }}
                  >
